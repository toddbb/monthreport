...
function computeSkillScore(skill, data, isVYL) {
   ...
   const hwVals = attended
      .filter((l) => skillFocuses.includes(l.info?.focus ?? "") && l.classes_seats?.homework?.hasHomework && l.classes_seats.homework.score != null)
...
const DATA_FILES = ["data/data1.json", "data/data2.json", "data/data3.json", "data/data4.json", "data/data5.json", "data/data6.json", "data/data7.json", "data/data8.json"];
...
for (const file of DATA_FILES) {
   ...
   const hwSubPoints = lessons
      .filter((l) => {
         ...
         const score = l.classes_seats?.homework?.hasHomework ? (l.classes_seats.homework.score ?? null) : null;
   ...
   const vocSubPoints = lessons
      .filter((l) => {
         if (!l.classes_seats?.isAttended) return false;
         const score = l.classes_seats?.vocabulary?.hasVocabulary ? (l.classes_seats.vocabulary.score ?? null) : null;
   ...
   const triggered = skillScores.filter((s) => s.score < thresholds.moderate).slice(0, maxRecs);
   ...
   if (recs.length === 0) {
      ...
      defaults.en.forEach((en, i) => recs.push({ en, vn: defaults.vn[i] ?? "", subPoints: [] }));
   }
